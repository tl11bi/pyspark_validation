    ...
    def validate(
        ...
        for idx, rule in enumerate(rules):
            ...
            if rtype not in SUPPORTED_TYPES:
                issues.append(self._err(rname, rtype, path, f"Unsupported type '{rtype}'. Supported: {list(SUPPORTED_TYPES)}"))
                continue
            if rname in names_seen:
                issues.append(self._err(rname, rtype, path, "Duplicate rule name"))
            names_seen.add(rname)
            if rtype == "headers": headers_count += 1

            # dispatch to per-rule method
            self.validators[rtype](rule, rname, rtype, path, dataset_columns, issues)
        ...
        # promote WARN -> ERROR if requested
        if fail_on_warning:
            issues = [ValidationIssue(i.rule_name, i.rule_type, i.path, ("ERROR" if i.level=="WARN" else i.level), i.message) for i in issues]
        return rules, issues
